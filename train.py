...
from data_augmentation import AugmentedDatasetMapper
import datasets as ds


class AugmentedTrainer(DefaultTrainer):
    """
    Custom trainer that uses AugmentedDatasetMapper when data augmentation is enabled.
    """
    
    @classmethod
    def build_train_loader(cls, cfg):
        """
        Build train loader with custom data mapper if data augmentation is enabled.
        """
        if getattr(cfg, 'DATA_AUGMENTATION', False):
            # Use augmented dataset mapper for training
            return build_detection_train_loader(
                cfg, 
                mapper=AugmentedDatasetMapper(cfg, is_train=True)
            )
        else:
...
def main():
    ...
    if not args.test_only:
        ...
        if cfg.ILOD.DISTILLATION and not cfg.ILOD.LOAD_TEACHER:
            print("Manually initializing teacher model with loaded weights")
            trainer.model.teacher_init()
...
def add_ilod_config(cfg):
    ...
    _C.DATA_AUGMENTATION = False
    _C.AUGMENTATION_FACTOR = 5
