...
VOC_CAT_IDS = ["car", "person", "motorbike"]

VOC_CAT_SHORT_NAMES = ["car", "person", "motor"]
...
def get_voc_dict(root_dir: str = VOC_ROOT, split: str = 'train') -> List[Dict[str, Any]]:
    ...
    train_imgs = os.path.join(root_dir, 'ImageSets/Custom', f'{split}.txt')
    train_img_ids = []
    ...
    for img_id in train_img_ids:
        annot_path = os.path.join(root_dir, 'Custom_Annotations', f'{img_id}.xml')
...
def _get_fine_tune(images_per_task: int):
    ...
    class_count = [0 for _ in range(3)]
