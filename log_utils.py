...
class InferenceDecorator:
    ...
    """
    def __init__(self, func, rpn_out=None, roi_out=None, nclasses=3):
        self.func = func
    ...
    def __call__(self, batched_inputs, **kwargs):
        ...
        if all("instances" in x for x in batched_inputs):
    ...
    def log_logits(self, batched_inputs, logits):
        ...
        with open(self.roi_out, 'a') as f:
            for box, cls, pred_logits in zip(instances.gt_boxes, instances.gt_classes, logits):
                if len(pred_logits) != (self.nclasses + 1):
                    raise ValueError(f"Expected {self.nclasses + 1} logits, but got {len(pred_logits)}")
